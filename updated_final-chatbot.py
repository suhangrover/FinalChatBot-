...
user_memory = {}  # Store personalized user details (e.g., name, preferences)
...
def handle_therapy_conversation(user_input):
    therapy_keywords = [
        "anxiety", "stress", "nervousness", "depression", "trauma", 
        "grief", "self-esteem", "relationship issues", "anger", 
        "burnout", "overwhelmed", "self-care", "negative thoughts", "loneliness"
    ]
...
def handle_general_conversation(user_input):
    general_responses = {
        ...
        "my name is": lambda name: f"Nice to meet you, {name}! I'll remember your name.",
    }

    normalized_input = user_input.lower().strip()
    
    # Handle custom user details like name
    if "my name is" in normalized_input:
        name = user_input.split("is")[-1].strip()
        user_memory["name"] = name
        return general_responses["my name is"](name)
...
def get_bot_response():
    ...
    conversation_box.insert(tk.END, f"Bot: I sense you're feeling {sentiment}.\n", "bot")
...
def recognize_speech():
    ...
    try:
        with mic as source:
            conversation_box.insert(tk.END, "Listening...\n", "bot")
            recognizer.adjust_for_ambient_noise(source)
            audio = recognizer.listen(source)

        user_input = recognizer.recognize_google(audio)
        conversation_box.insert(tk.END, f"You (via speech): {user_input}\n", "user")
        entry.delete(0, tk.END)
        entry.insert(0, user_input)  # Insert recognized text into the entry box
        get_bot_response()
    except sr.UnknownValueError:
        conversation_box.insert(tk.END, "Bot: Sorry, I couldn't understand your speech.\n", "bot")
    except sr.RequestError as e:
        conversation_box.insert(tk.END, f"Bot: Could not request results from speech service; {e}\n", "bot")
...
speech_button = ttk.Button(entry_frame, text="Speak", command=recognize_speech, style="SendButton.TButton")
speech_button.pack(side=tk.RIGHT, padx=10)
